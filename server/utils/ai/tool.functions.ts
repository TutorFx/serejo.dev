...
export function experienceSearchTool(event: H3Event) {
  return tool(
    async () => {
...
export function formFillingTool(event: H3Event) {
  return tool(
    async (input) => {
      const prisma = usePrisma()
      const guest = await setupGuest(event)
      if (!guest)
        return 'Error: invalid guest id (not your fault)'
      const partialUser = userSchema.safeParse(input)
      if (!partialUser.data || partialUser.error)
        return 'Error on parse query'
...
export function getUserDataTool(event: H3Event) {
  return tool(
    async () => {
      const prisma = usePrisma()
      const guest = await setupGuest(event)
