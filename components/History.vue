...
<template>
  <div v-if="(service instanceof HistoryService)" class="grid py-12 lg:grid-cols-3 sm:grid-cols-2 gap-x-4 gap-y-6">
    <NuxtLink v-for="item in service.getRepository()" :key="item._id" :to="localePath({ name: `experience-item`, params: { item: item.org } })" class="grid aspect-video p-4 bg-base-100 rounded-md group">
